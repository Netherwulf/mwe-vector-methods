...
def load_mwes(correct_mwes_file, incorrect_mwes_file):
    ...
    correct_mwe_list, lemmatized_correct_mwes = clean_mwe_list(correct_mwe_list, lemmatized_correct_mwes)
    incorrect_mwe_list, lemmatized_incorrect_mwes = clean_mwe_list(incorrect_mwe_list, lemmatized_incorrect_mwes)
...
def get_sentences_containing_mwe(output_file, correct_mwes, incorrect_mwes, lemmatized_correct_mwes,
    ...
    for sentence_ind, sentence in enumerate(sentences_lemmas):
        ...
        if len(sentences_orths[sentence_ind]) != len(sentences_lemmas[sentence_ind]):
            print(f'sentence orths: {sentences_orths[sentence_ind]}',
                  f'sentence lemmas: {sentences_lemmas[sentence_ind]}',
                  sep='\n')
        if '/' in sentence:
            continue
        ...
        for lemma_ind, lemma in enumerate(sentence):
            ...
            if not word_in_complete_mwe_list[lemma_ind] and lemma_ind != len(sentence) - 1:
                matching_corr_mwes = [corr_mwe for corr_mwe in lemmatized_correct_mwes if
                                      ' '.join([sentence[lemma_ind], sentence[lemma_ind + 1]]) == corr_mwe]

                matching_incorr_mwes = [incorr_mwe for incorr_mwe in lemmatized_incorrect_mwes if
                                        ' '.join([sentence[lemma_ind], sentence[lemma_ind + 1]]) == incorr_mwe]
            ...
            if not word_in_complete_mwe_list[lemma_ind]:
                ...
                matching_incorr_mwes = [incorr_mwe for incorr_mwe in lemmatized_incorrect_mwes if
                                        lemma == incorr_mwe.split(' ')[0] or lemma == incorr_mwe.split(' ')[1]]
